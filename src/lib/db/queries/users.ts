...
export async function getUserById(id: string): Promise<User | null> {
  const rows = await sql<User[]>`
    SELECT
      id, email, email_verified AS "emailVerified",
      display_name AS "displayName", cep,
...
export async function updateUserProfile(
  ...
  const rows = await sql<User[]>`
    ...
    RETURNING
      ...
      display_name AS "displayName", cep,
      whatsapp, whatsapp_opt_in AS "whatsappOptIn",
      avatar_url AS "avatarUrl", age_group AS "ageGroup",
      account_status AS "accountStatus", created_at AS "createdAt"
  `;
  return rows[0] ?? null;
}
...
export async function completeOnboarding(
  id: string,
  data: { cep: string; whatsapp?: string; whatsappOptIn: boolean }
): Promise<void> {
  await sql`
    UPDATE users SET
      cep             = ${data.cep},
